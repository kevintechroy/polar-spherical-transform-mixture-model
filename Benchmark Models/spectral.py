...
def run_spectral_pipeline(dataset_name, dataset_path, target_col=None, test_size=0.5, random_state=42):
    ...
    print(f"[INFO] Training Spectral Clustering (n_clusters={num_classes})...")
    spectral = SpectralClustering(n_clusters=num_classes, affinity='nearest_neighbors', assign_labels='kmeans', random_state=random_state)
    cluster_labels_train = spectral.fit_predict(X_train)

    print("       Predicting test labels using NearestCentroid proxy...")
    centroid_model = NearestCentroid()
    centroid_model.fit(X_train, cluster_labels_train)
    cluster_labels_test = centroid_model.predict(X_test)
    ...
    label_mapping = map_clusters_to_labels(y_test, cluster_labels_test)
    y_pred = np.array([label_mapping.get(c, -1) for c in cluster_labels_test])
    ...
    print("\n" + "="*40)
    print(" MODEL RESULTS (Spectral Clustering)")
    print("="*40)
    ...
    plt.title(f"Confusion Matrix (Spectral)\nAcc: {acc*100:.2f}% on {dataset_name}")
...
if __name__ == "__main__":
    ...
    run_spectral_pipeline(dataset_name_input, os.path.normpath(dataset_path))
